...
class Window(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.saveLowButton = QPushButton()
        self.saveLowButton.setMinimumSize(QSize(0, 0))
        self.saveLowButton.setMaximumSize(QSize(88, 21))
        self.saveLowButton.setObjectName("saveLowButton")
        self.ui.gridLayout.addWidget(self.saveLowButton, 4, 0, 1, 1)
        self.saveLowButton.setText("Save Low")
        
        self.saveHighButton = QPushButton()
        self.saveHighButton.setMinimumSize(QSize(0, 0))
        self.saveHighButton.setMaximumSize(QSize(88, 21))
        self.saveHighButton.setObjectName("saveHighButton")
        self.ui.gridLayout.addWidget(self.saveHighButton, 4, 1, 1, 1)
        self.saveHighButton.setText("Save High")
        ...
        # self.ui.gridLayout.addWidget(self.calibrateButton, 5, 0, 1, 2)
    ...
    def initalConnections(self):
        ...
        self.saveLowButton.clicked.connect(self.saveLowButtonPushed)
        self.saveHighButton.clicked.connect(self.saveHighButtonPushed)
    ...
    def plotbuttonPushed(self):
        ...
        T0 = 25+273.15 # K
        K = 3950 # K
        self.A = 1.0/(1.0/T0+np.log(self.R1/self.R01*self.A/((2**12-1)-self.A))/self.K1) # oC
        self.B = 1.0/(1.0/T0+np.log(self.R2/self.R02*self.B/((2**12-1)-self.B))/self.K2) # oC
        
        # Correction
        if self.lowSaved and self.highSaved:
            self.A = (self.A-self.low1)/(self.high1-self.low1)*(self.T2-self.T1)+self.T1
            self.B = (self.B-self.low2)/(self.high2-self.low2)*(self.T2-self.T1)+self.T1
        
        self.A = self.A-273.15
        self.B = self.B-273.15
    ...
    def saveLowButtonPushed(self):
        self.A = self.data['A'].to_numpy()
        self.B = self.data['B'].to_numpy()
        
        T0 = 25+273.15 # K
        K = 3950 # K
        self.A = 1.0/(1.0/T0+np.log(self.R1/self.R01*self.A/((2**12-1)-self.A))/K) # oC
        self.B = 1.0/(1.0/T0+np.log(self.R2/self.R02*self.B/((2**12-1)-self.B))/K) # oC
        
        self.low1 = np.mean(self.A)
        self.low2 = np.mean(self.B)
        
        self.lowSaved = 1
        
        print(self.low1,self.low2,self.T1)
        print('Low Calibration Data Saved!')

    def saveHighButtonPushed(self):
        self.A = self.data['A'].to_numpy()
        self.B = self.data['B'].to_numpy()
        ...
        self.high1 = np.mean(self.A)
        self.high2 = np.mean(self.B)
        
        self.highSaved = 1
        
        print(self.high1,self.high2,self.T2)
        print('High Calibration Data Saved!')
    ...
    def settingsPushed(self):
        ...
        T1_label = QLabel()
        T1_label.setText("Cal. Temp. Low (oC)")
        T1_widget = QLineEdit(self.settingsDialog)
        T1_widget.setText(str(self.T1-273.15))
        T1_layout.addWidget(T1_label)
        T1_layout.addWidget(T1_widget)
        layout.addLayout(T1_layout)
        
        T2_layout = QHBoxLayout()
        T2_label = QLabel()
        T2_label.setText("Cal. Temp. High (oC)")
        T2_widget = QLineEdit(self.settingsDialog)
        T2_widget.setText(str(self.T2-273.15))
        T2_layout.addWidget(T2_label)
        T2_layout.addWidget(T2_widget)
        ...
        if self.settingsDialog.exec_() == QDialog.Accepted:
            ...
            self.T1 = float(T1_widget.text())+273.15
            self.T2 = float(T2_widget.text())+273.15
